...
This guide will walk you through deploying a static site to GitHub Pages. Static sites are apps that consist of fixed content (HTML, CSS, JavaScript, and static assets such as images) that is delivered to users exactly as it is stored.
...
- An entry file for your site. By default GitHub Pages will look for an `index.html` file in the root of your application as the entry file.
...
On the GitHub Pages view, you'll select a branch you'd like to deploy to enable GitHub Pages for this repository. First, click the dropdown labeled **None** under the **Branch** header outlined in red below. Next, select the branch you'd like to deploy. You can deploy any branch - only changes in that branch will be deployed. As shown here, we'll be deploying the `main` branch outlined in red below, but any branch could be deployed.
...
As we noted earlier, the entry point into our application is an `index.html` file in the root of our app so we don't need to change where the site will build from (**/ (root)**). Click the **Save** button as outlined in red below.
...
Once deployed, you can find a link to the site on this same page shown below outlined in red. Deploying will typically happen within five minutes but could take up to ten. If it takes longer, try pushing changes to the branch you selected to deploy the app from, and again wait 5-10 minutes for the site to deploy.
...
Each push to the branch you are using to deploy the site on GitHub will trigger an update to the deployed site. While most of the time, changes will be live within 5 minutes, it could take up to 10 minutes for the deployed site to update.

## Troubleshooting

If your site has never successfully deployed, start by confirming you are publishing the correct branch and that you have specified the correct directory as the entry point.

Once you've confirmed this, most errors or missing updates can be resolved by committing and pushing to the branch you are using to deploy the site. Attempt this first.

If you're still having errors, they can likely be diagnosed within the **Actions** page in the GitHub repo, shown in the screenshot below. Pair with an instructor to help troubleshoot.
