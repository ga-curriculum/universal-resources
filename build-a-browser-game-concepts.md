# Build a Browser Game: Concepts

Project planning is an essential part of the development process. In this resource, you'll learn how to plan a browser game project using a user stories, wireframes, and pseudocode.

We will be using the `JavaScript Browser Game - Rock, Paper, Scissors` lesson as an example for this resource.

## User Stories

A user story is a short, simple description of a feature told from the perspective of the person who desires the new capability, in our case this will be a user of our game.

These are often the first things we write when planning a project. During our project we will be writing user stories to include all project requirements and help us understand what features we need to build and why we are building them.

User stories are written in the format:

> As a [type of user], I want [an action] so that [a benefit/a reason].

The last part of the user story is optional, but it can be helpful to include the reason why a user wants to perform a certain action. However if the reason is clear from the user story, it can be omitted.
...
When writing user stories, it's important to keep them simple and concise. We want to focus on the user's needs and not get bogged down in the technical details of how we will implement the feature. 

It's best to think through the game as if you are the user going to play the game. What do you want to see? What do you want to do? What do you want to happen? The above user stories are a good example of this.
...
We can write our pseudocode in any way that make sense to us. We can use bullet points, or we can write it out in paragraph form. The important thing is that we are able to understand it.

Some useful tips to get you started with pseudocode:
...
```js
// IF user clicks on rock button
  // THEN computer chooses rock, paper, or scissors
```
...
5. Invoke the init function used to initalize all state variables.
6. Invoke the main render function responsible for tranfering all state variables to the DOM.
7. Wait for user to click on a button.
   - Update all state variables with the correct values depending on the user's choice.
   - Invoke the main render function.
8. Wait for user to click on the "Play Again" button.
...
```js
// Compair the user's choice to the computer's choice
...
The above is just a loose breakdown of the steps we will need to take to build our game. As we start to write our code, we will likely find that we need to add more steps or change the order of the steps. That's okay! Pseudocode is meant to be flexible and help us plan out our game. It's not set in stone.
